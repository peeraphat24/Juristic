...
<link rel="shortcut icon" type="image/x-icon" href="pic/icon.ico">
  <head>
    ...
    <title>รายการบันทึกแบบขอต่อเติมบ้าน</title>
...
?><br>
        <center> <h3>รายการบันทึกแบบขอต่อเติมบ้าน</h3>
        <table>
          <tr>
            <form class="" action="buildingdata.php" method="post">
        ...
        ?>
             <div class="container">
              ...
              if(!$con) die ("ไม่สามารถติดต่อ mysql ได้");
                ...
                $strSQL = "SELECT * FROM building bu,member mem where bu.member_id = mem.member_id and (mem.m_firstname like '%".$search."%' or mem.m_lastname like '%".$search."%') ";
...
$strSQL .=" order by build_id ASC LIMIT $Page_Start , $Per_Page";
...
$offset=($Page-1)*$Per_Page; // ตรงนี้สำคัญครับ
              ...
              <tr class="text-dark bg-white">
                ...
                <td align="center"><?php echo $id2 = $objResult["build_id"];?></td>
                <td><?php
                    $host = "localhost";
                    $user = "root";
                    $pass = "12345678";
                ...
                ?></td>
                <td align="center"><?php echo date("d/m/Y", strtotime($objResult["build_startdate"]));;?></td>
                <td align="center"><?php echo date("d/m/Y", strtotime($objResult["build_enddate"]));;?></td>
                <td align="center"><?php echo date("d/m/Y", strtotime($objResult["date"]));;?></td>
                <td align="center"><?php echo $objResult["telephone"]; ?></td>
                <td  align="center"><?php if($objResult['b_status'] == "2"){?>
                  <font color="red" title="ไม่อนุญาต"><i class="fas fa-clipboard-check"></i></font>
                <?}
                else if($objResult['b_status'] == "1"){?>
                  <font color="Green" title="อนุญาต"><i class="fas fa-clipboard-check"></i></font>
              <?php }elseif($objResult['b_status'] == "0"){ ?>
                    <font color="yellow" title="รอการพิจารณา"><i class="fas fa-clipboard-check"></i></font>
              <?}?>
              </td>
                <td><a href="#detail<?php echo $objResult["build_id"];?>" data-toggle="modal">
                  <!-- Button trigger modal -->
                <button type="button" class="btn btn-info view_data" value="รายละเอียด">
                <i class="fas fa-info-circle"> รายละเอียด</i>
                </button></a>
                <!-- Modal -->
                <div class="modal fade bd-example-modal-lg" id="detail<?php echo $objResult["build_id"];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
                      ...
                      <center>
                        <table class="">
                          <tr>
                              ...
                              <td align="center"><?php echo $objResult["m_address"];?></td>
                          </tr>
                          <tr>
                            <td></td>
                            <td></td>
...
if($Prev_Page)
{
	echo " <a href='buildingdata.php?Page=$Prev_Page'><< Back</a> ";
}

for($i=1; $i<=$Num_Pages; $i++){
	if($i != $Page)
	{
		echo " <a href='buildingdata.php?Page=$i'>$i</a> ";
	}
	else
	{
		echo "[<b> $i </b>]";
...
if($Page!=$Num_Pages)
{
	echo " <a href ='buildingdata.php?Page=$Next_Page'>Next>></a> ";
}
