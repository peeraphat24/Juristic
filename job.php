...
?><br>
<center> <h3>รายการตำแหน่ง</h3><br>
  <table>
    <tr>
      <form class="" action="job.php" method="post">
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h3 class="modal-title" id="exampleModalLongTitle">เพิ่มตำแหน่ง</h3>
          ...
          <div class="modal-body">
            ...
            <table>
              <tr>
              <td>ชื่อตำแหน่ง<font color="red">*&nbsp;</font></td>
...
<center>
       <div class="container">
        ...
        <?php
          ...
          $strSQL = "SELECT * FROM job where (job.job_name like '%".$search."%')";
  ...
  while($objResult = mysql_fetch_array($objQuery)){
      $count++;
        ...
        <tr align="center" class="text-dark bg-white">
          ...
          <td align="left"><?php echo $objResult["job_name"];?></td>
        	<td><a href="#delete<?php echo $objResult["job_id"];?>" data-toggle="modal">
            <button type="button" class="btn btn-danger" value="ลบ" title="ลบข้อมูล">
            <i class="fas fa-minus-circle"></i>
            </button></a>
            <div class="modal fade" id="delete<?php echo $objResult["job_id"];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                  ...
                  <div class="modal-body">
                    <form action="deletejob.php?id=<?php echo $objResult["job_id"];?>" method="post">
                      คุณต้องการลบข้อมูลนี้หรือไม่
                  </div>
                  <div class="modal-footer">
                    <button type="submit" class="btn btn-success" value="<?php echo $objResult["job_id"];?>"><i class="fas fa-eraser"></i> ลบ</button>
                  </div>
                  </form>
                </div>
              </div>
            </div>
          <a href="#edit<?php echo $objResult["job_id"];?>" data-toggle="modal">
            <button type="button" class="btn btn-dark" value="แก้ไข" title="แก้ไขข้อมูล">
              <i class="fas fa-cogs"></i>
            </button></a>
            <div class="modal fade" id="edit<?php echo $objResult["job_id"];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                  ...
                  <div class="modal-body">
                    <form action="editjob2.php?id=<?php echo $objResult["job_id"];?>" method="post">
                      <table>
                              <tr>
                              <td>ชื่อตำแหน่ง &nbsp;</td>
                              <td><input type="text" value="<?php echo $objResult["job_name"];?>" size="70" name="type" class="form-control"></td>
                            </tr>
                      </table>
                  </div>
                  <div class="modal-footer">
                    <input type="hidden" name="id" value="<?php echo $objResult["job_id"];?>">
                    <button type="submit" value="<?php echo $objResult["job_id"];?>" class="btn btn-success"><i class="fas fa-check"></i> ยืนยันการแก้ไข</button>
                  </div>
                </div>
              </div>
            </div>
    ...
    if($Prev_Page)
    {
      echo " <a href='job.php?Page=$Prev_Page'><< Back</a> ";
    }
    ...
    if($Page!=$Num_Pages)
    {
      echo " <a href ='job.php?Page=$Next_Page'>Next>></a> ";
    }
