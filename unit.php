...
<link rel="shortcut icon" type="image/x-icon" href="pic/icon.ico">
  <head>
    ...
    <title>จัดการข้อมูลหน่วยนับ</title>
...
<body>
  ...
  <div class="wrap">
	...
	<?php
      ...
      $strSQL .=" order  by unit_id ASC LIMIT $Page_Start , $Per_Page";
      $objQuery  = mysql_query($strSQL);
      $offset=($Page-1)*$Per_Page; // ตรงนี้สำคัญครับ
      $count=$offset;
      ?><br>
<center> <h3>รายการประเภทจำนวนนับ</h3><br>
  <table>
    <tr>
      <form class="" action="unit.php" method="post">
        <td><input type="text" name="search" value="<?=$_POST['search']?>" placeholder="ชื่อหน่วยนับ" class="form-control"></td>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h3 class="modal-title" id="exampleModalLongTitle">เพิ่มหน่วยนับ</h3>
          ...
          <div class="modal-body">
            ...
            <table>
              <tr>
              <td>ชื่อหน่วยนับ<font color="red">*&nbsp;</font></td>
...
<center>
 ...
 $strSQL = "SELECT * FROM  unit where (unit.unit_name like '%".$search."%')";
  ...
  while($objResult = mysql_fetch_array($objQuery)){
      $count++;
        ...
        <tr align="center" class="text-dark bg-white">
        	...
        	<td><?php echo $objResult["unit_id"];?></td>
          <td align="left"><?php echo $objResult["unit_name"];?></td>
        	<td><a href="#delete<?php echo $objResult["unit_id"];?>" data-toggle="modal">
            <button type="button" class="btn btn-danger" value="ลบ" title="ลบข้อมูล">
          <i class="fas fa-minus-circle"></i>
          </button></a><div class="modal fade" id="delete<?php echo $objResult["unit_id"];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                  <form action="deleteunit.php?id=<?php echo $objResult["unit_id"];?>" method="post">
                    คุณต้องการลบข้อมูลนี้หรือไม่
                </div>
                <div class="modal-footer">
                  <button type="submit" class="btn btn-success" value="<?php echo $objResult["unit_id"];?>"><i class="fas fa-eraser"></i> ลบ</button>
                </div>
                </form>
              </div>
            </div>
          </div>
          <a href="#edit<?php echo $objResult["unit_id"];?>" data-toggle="modal">
            <button type="button" class="btn btn-dark" value="แก้ไข" title="แก้ไขข้อมูล">
                <i class="fas fa-cogs"></i>
            </button></a>
            <div class="modal fade" id="edit<?php echo $objResult["unit_id"];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                  ...
                  <div class="modal-body">
                    <form action="editunit2.php?id=<?php echo $objResult["unit_id"];?>" method="post">
                      <table>
                              <tr>
                              <td>ชื่อหน่วยนับ &nbsp;</td>
                              <td><input type="text" value="<?php echo $objResult["unit_name"];?>" size="30" name="type" class="form-control"></td>
                              </tr>
                      </table>
                    </div>
                    <div class="modal-footer">
                      <input type="hidden" name="id" value="<?php echo $objResult["unit_id"];?>">
                      <button type="submit" value="<?php echo $objResult["unit_id"];?>" class="btn btn-success"><i class="fas fa-check"></i> ยืนยันการแก้ไข</button>
                    </div>
                  </div>
                </div>
              </div>
    ...
    if($Prev_Page)
    {
      echo " <a href='unit.php?Page=$Prev_Page'><< Back</a> ";
    }

    for($i=1; $i<=$Num_Pages; $i++){
      if($i != $Page)
      {
        echo " <a href='unit.php?Page=$i'>$i</a> ";
      }
      else
      {
        echo "[<b> $i </b>]";
    ...
    if($Page!=$Num_Pages)
    {
      echo " <a href ='unit.php?Page=$Next_Page'>Next>></a> ";
    }
